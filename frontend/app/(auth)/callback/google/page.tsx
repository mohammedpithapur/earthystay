...
export default function GoogleCallbackPage() {
  ...
  const { login } = useAuth() // we don't need login directly — we call backend and use the returned token
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/auth/google/callback?code=${encodeURIComponent(code)}`, {
      ...
      .then(async res => {
        ...
        // Backend set the refresh cookie — now silently refresh to load user into context
        // The easiest approach: trigger a page redirect; AuthContext will restore session on load
        router.replace(next)
      })
      .catch(err => {
