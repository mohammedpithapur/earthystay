  ...
  return (
    <div style={{ backgroundColor: '#ffffff', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: 'clamp(32px, 5vw, 64px)', fontWeight: '800', color: '#1a1a1a', marginBottom: '24px', lineHeight: '1.15', maxWidth: '800px', margin: '0 auto 24px' }}>
        ...
        <p style={{ fontSize: '16px', color: '#555', maxWidth: '580px', margin: '0 auto', lineHeight: '1.8' }}>
      ...
      <section style={{ padding: 'clamp(64px, 8vw, 100px) 24px', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '48px', alignItems: 'center' }}>
          <div>
            <p style={{ fontSize: '12px', letterSpacing: '3px', textTransform: 'uppercase', color: '#e1c391', fontWeight: '600', marginBottom: '16px' }}>
              How It Started
            </p>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 38px)', fontWeight: '800', color: '#1a1a1a', marginBottom: '24px', lineHeight: '1.2' }}>
          ...
          {/* Visual placeholder */}
          <div style={{
            backgroundColor: '#ede8d0', borderRadius: '12px',
            height: '380px', display: 'flex', alignItems: 'center',
            justifyContent: 'center', flexDirection: 'column', gap: '12px'
          }}>
            <div style={{ fontSize: '64px', color: '#e1c391', fontWeight: '800', letterSpacing: '-2px' }}>ES</div>
            <p style={{ fontSize: '13px', color: '#888', letterSpacing: '3px', textTransform: 'uppercase' }}>Est. 2022</p>
          </div>
        </div>
      </section>

      {/* Stats */}
      <section style={{ backgroundColor: '#1a1a1a', padding: 'clamp(48px, 6vw, 80px) 24px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0' }}>
          {stats.map((stat, i) => (
            <div key={stat.label} style={{
              ...
              <p style={{ fontSize: 'clamp(36px, 5vw, 52px)', fontWeight: '800', color: '#e1c391', marginBottom: '8px', lineHeight: '1' }}>
                {stat.number}
              </p>
      ...
      <section style={{ padding: 'clamp(64px, 8vw, 100px) 24px', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '64px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: '800', color: '#1a1a1a', marginBottom: '16px' }}>
            Our Values
          </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px' }}>
          {values.map((value, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: '28px', color: '#e1c391', marginBottom: '16px' }}>{value.symbol}</div>
              <div style={{ width: '32px', height: '2px', backgroundColor: '#e1c391', marginBottom: '16px' }} />
              <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a1a', marginBottom: '12px' }}>{value.title}</h3>
              <p style={{ fontSize: '14px', color: '#555', lineHeight: '1.8' }}>{value.description}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            {team.map((member, i) => (
              <div key={i} style={{
                ...
                padding: '32px 28px', border: '1px solid #e0d9c0', textAlign: 'center'
              }}>
                <div style={{
                  width: '72px', height: '72px', backgroundColor: '#e1c391',
                ...
                <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#1a1a1a', marginBottom: '6px' }}>{member.name}</h3>
                <p style={{ fontSize: '12px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#e1c391', fontWeight: '600', marginBottom: '16px' }}>{member.role}</p>
                <div style={{ width: '32px', height: '2px', backgroundColor: '#e0d9c0', margin: '0 auto 16px' }} />
                <p style={{ fontSize: '14px', color: '#555', lineHeight: '1.7' }}>{member.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: 'clamp(64px, 8vw, 100px) 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '15px', color: '#555', marginBottom: '32px', maxWidth: '480px', margin: '0 auto 32px', lineHeight: '1.7' }}>
          Browse our handpicked collection of luxury properties across India&aposs most beautiful destinations.
        </p>
        <Link
          href="/properties"
          style={{
            display: 'inline-block', backgroundColor: '#e1c391',
            color: '#1a1a1a', padding: '18px 48px', fontSize: '13px',
