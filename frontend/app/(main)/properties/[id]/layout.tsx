...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(${API_BASE}/properties/, { next: { revalidate: 3600 } })
    if (!res.ok) throw new Error('Not found')
    const p = await res.json()
    const title = p.name || 'Property | EarthyStay'
    const description =
      p.short_description || p.description?.slice(0, 160) || 'A handpicked earthy stay.'
    ...
    const canonicalUrl = https://earthystays.in/properties/
    return {
      title,
      description,
      ...
      openGraph: {
        ...
        description: location ? ${description} Located in . : description,
